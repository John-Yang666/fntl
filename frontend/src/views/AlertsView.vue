<template>
  <section class="alerts-view" data-testid="alerts-view">
    ...
    <div class="filter-container">
      ...
      >
        批量确认（{{ selectedConfirmable.length }}）
      </el-button>
    </div>
      ...
      <table>
          ...
          <tr>
            <th class="check-column">
              <input
                ...
                :disabled="visibleConfirmable.length === 0"
                aria-label="选择全部可确认告警"
        ...
        <tbody>
          <tr v-for="(alert, index) in filteredAlerts" :key="alert.uniqueKey">
            <td class="check-column">
              <input
                ...
                :aria-label="`选择${alert.device_name}告警`"
              />
            </td>
            <td>{{ index + 1 }}</td>
            <td>{{ alert.system.toUpperCase() }}</td>
            <td>{{ alert.source === 'current' ? '当前' : '历史' }}</td>
          ...
          <tr v-if="!loading && filteredAlerts.length === 0">
            <td colspan="12" class="empty-row">暂无符合条件的告警</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const visibleConfirmable = computed(() => filteredAlerts.value.filter((alert) => !alert.confirmed));
const selectedConfirmable = computed(() => {
  const keys = new Set(selectedKeys.value);
  return alerts.value.filter((alert) => !alert.confirmed && keys.has(alert.uniqueKey));
});
const allVisibleConfirmableSelected = computed(() =>
  visibleConfirmable.value.length > 0 &&
  visibleConfirmable.value.every((alert) => selectedKeys.value.includes(alert.uniqueKey)),
);

const toggleAllVisible = (event: Event) => {
  const checked = (event.target as HTMLInputElement).checked;
  const visibleKeys = new Set(visibleConfirmable.value.map((alert) => alert.uniqueKey));
...
const confirmSelected = () => confirmAlerts(selectedConfirmable.value);
