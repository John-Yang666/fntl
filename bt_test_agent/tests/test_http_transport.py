...
from bt_test_agent.cli import HttpClient
...
class HttpClientComposeTransportTests(unittest.TestCase):
    def test_compose_transport_executes_request_inside_web_container(self):
        ...
        with mock.patch("bt_test_agent.cli.subprocess.run", return_value=completed) as run:
            client = HttpClient(
                "http://127.0.0.1:8000",
                "admin",
                "admin",
        ...
        self.assertEqual(command[:7], ["docker", "compose", "-f", "docker-compose-prod.yml", "exec", "-T", "web"])
