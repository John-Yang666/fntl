...
SYSTEM = "bt"
ALARM_GROUP = "bt_alarm_state_updates"
REVISION_CACHE_KEY = "bt:alarm-state-revision"
...
class MonitoringPreferenceView(APIView):
    ...
    def get(self, request):
        ...
        effective_ids = monitored_device_ids_for_user(request.user)
        return Response({"selection_mode": preference.selection_mode, "device_ids": effective_ids})
    ...
    def put(self, request):
        ...
        return Response({
            ...
            "device_ids": monitored_device_ids_for_user(request.user),
        })
...
class AlarmConfirmationsView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            current_count = AlarmActive.objects.filter(
                id__in=current_ids,
                device_id__in=monitored_ids,
                is_confirmed=False,
            ).update(is_confirmed=True)
            historical_count = AlarmData.objects.filter(
                id__in=historical_ids,
                device_id__in=monitored_ids,
                is_confirmed=False,
            ).update(is_confirmed=True)
