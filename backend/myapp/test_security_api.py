...
@override_settings(MEDIA_ROOT="/tmp/bt-nms-test-media")
class SecurityApiTests(APITestCase):
    ...
    @patch("myapp.views.send_packet")
    def test_send_command_requires_authentication(self, mock_send_packet):
        response = self.client.post(
            reverse("send-command", args=[self.device_a.device_id]),
            {
    ...
    @patch("myapp.views.send_packet")
    def test_send_command_rejects_devices_outside_user_scope(self, mock_send_packet):
        self.client.force_authenticate(self.ops_user)

        response = self.client.post(
            reverse("send-command", args=[self.device_b.device_id]),
            {
                "function_code": 1,
                "time": 0,
                "operation": 2,
    ...
    @patch("myapp.views.send_packet")
    def test_send_command_uses_authenticated_user_for_audit(self, mock_send_packet):
        self.client.force_authenticate(self.ops_user)
