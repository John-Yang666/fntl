...
set "APP_DIR=sy_agent_ui"
set "APP_EXE=sy_agent_ui.exe"
set "APP_LABEL=sy_agent_ui"

call :resolve_root
if errorlevel 1 exit /b 1

if not defined SY_UI_AGENT_RESTART_STABLE_RUN_SEC set "SY_UI_AGENT_RESTART_STABLE_RUN_SEC=60"
if not defined SY_UI_AGENT_RESTART_BACKOFF_MAX_SEC set "SY_UI_AGENT_RESTART_BACKOFF_MAX_SEC=30"
...
echo [INFO] Restart loop enabled. Set SY_UI_AGENT_RUN_ONCE=1 to disable.
...
if !RUN_UPTIME_SEC! GEQ %SY_UI_AGENT_RESTART_STABLE_RUN_SEC% (
  set /a RESTART_STREAK=0
) else (
  set /a RESTART_STREAK+=1
)
call :calc_backoff !RESTART_STREAK! %SY_UI_AGENT_RESTART_BACKOFF_MAX_SEC% RESTART_DELAY_SEC
if !RUN_UPTIME_SEC! GEQ %SY_UI_AGENT_RESTART_STABLE_RUN_SEC% (
