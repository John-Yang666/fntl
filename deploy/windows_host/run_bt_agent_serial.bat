...
set "APP_DIR=bt_agent_serial"
set "APP_EXE=bt_agent_serial.exe"
set "APP_LABEL=bt_agent_serial"

call :resolve_root
if errorlevel 1 exit /b 1

if not defined BT_AGENT_SERIAL_RESTART_STABLE_RUN_SEC set "BT_AGENT_SERIAL_RESTART_STABLE_RUN_SEC=60"
if not defined BT_AGENT_SERIAL_RESTART_BACKOFF_MAX_SEC set "BT_AGENT_SERIAL_RESTART_BACKOFF_MAX_SEC=30"
...
echo [INFO] Restart loop enabled. Set BT_AGENT_SERIAL_RUN_ONCE=1 to disable.
...
if /i "%BT_AGENT_SERIAL_RUN_ONCE%"=="1" (
...
if !RUN_UPTIME_SEC! GEQ %BT_AGENT_SERIAL_RESTART_STABLE_RUN_SEC% (
  set /a RESTART_STREAK=0
) else (
  set /a RESTART_STREAK+=1
)
call :calc_backoff !RESTART_STREAK! %BT_AGENT_SERIAL_RESTART_BACKOFF_MAX_SEC% RESTART_DELAY_SEC
if !RUN_UPTIME_SEC! GEQ %BT_AGENT_SERIAL_RESTART_STABLE_RUN_SEC% (
